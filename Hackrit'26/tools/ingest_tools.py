"""Static analysis and ingestion tools for RepoRamp.
...
def clone_repo(
    ...
    if not target_dir:
        target_dir = os.path.join(os.getcwd(), "workspaces", repo_name)
    
    target_path = Path(target_dir).resolve()
    
    # Check if directory already exists with repo
    if target_path.exists() and (target_path / ".git").is_dir():
        ...
        try:
            ...
            # Resilient fallback for local testing or simulated repos
...
def build_graph_digest(
    ...
    # Check if AST is already in session state
    ...
    # If graph is empty (e.g. initial empty test repo), add a default root node
    if G.number_of_nodes() == 0:
        G.add_node("file::main.py", type="file", name="main.py")

    # Louvain community detection via NetworkX built-in
    try:
    ...
    # Store in session state for downstream reuse
    if tool_context and hasattr(tool_context, "state"):
...
# Wrap as ADK FunctionTool primitives

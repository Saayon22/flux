# Local code reading, writing, and editing tools for Google ADK agents.
...
def read_file(file_path: str, tool_context: Optional[ToolContext] = None) -> Dict[str, Any]:
    ...
    try:
        return {"status": "success", "file_path": str(path), "content": path.read_text(encoding="utf-8", errors="ignore")}
...
def write_file(file_path: str, content: str, tool_context: Optional[ToolContext] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"status": "success", "message": f"Successfully wrote {len(content)} characters to {path}", "file_path": str(path)}
    except Exception as e:
        return {"status": "error", "error": str(e)}


# Edits a local file by replacing a target snippet with a replacement snippet.
def edit_file(
    file_path: str,
    target_snippet: str,
    ...
    try:
        ...
        if target_snippet not in content:
            return {"status": "error", "error": "Target snippet not found in file."}
        new_content = content.replace(target_snippet, replacement_snippet, 1)
        path.write_text(new_content, encoding="utf-8")
        return {"status": "success", "message": f"Successfully edited {path}", "file_path": str(path)}
    except Exception as e:
        return {"status": "error", "error": str(e)}


# Lists files and folders within a target directory.
def list_files(directory_path: str = ".", tool_context: Optional[ToolContext] = None) -> Dict[str, Any]:
    ...
    try:
        entries = [{"name": item.name, "is_dir": item.is_dir(), "size_bytes": item.stat().st_size if item.is_file() else 0} for item in sorted(path.iterdir())]
